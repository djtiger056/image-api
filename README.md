...
```
 ...
 你本地是 Windows，没有 systemd，用 git-bash 手动管理。以下是常用命令：

      ================================================================
          image-api 服务管理命令 (PowerShell)
        ================================================================
        ...
        【重启】
          Get-Process node -ErrorAction SilentlyContinue | Stop-Process -Force
          cd E:\MyProject\hermescodes\image-api; npm run build; npm run start
    
        【查看日志】
          Get-Content E:\MyProject\hermescodes\image-api\logs\$(Get-Date -Format 'yyyy-MM-dd').log -Tail 50
    
        实时跟踪（类似 tail -f）
          Get-Content E:\MyProject\hermescodes\image-api\logs\$(Get-Date -Format 'yyyy-MM-dd').log -Wait
    
        【健康检查】
          Invoke-WebRequest http://localhost:8000/ping -UseBasicParsing | Select-Object -ExpandProperty Content
    
        ================================================================
    
        或者你也可以在 PowerShell 里直接进 git-bash 环境：
    
          bash
    
        然后就能用之前给你的 Linux 风格命令了（grep, curl, tail 等）。输入 exit 退回 PowerShell。xxxxxxxxxx 
