...
function getConfiguredApiKeys(): string[] {
    const raw = process.env.SERVER_API_KEYS || process.env.SERVER_API_KEY || '';
...
function verifyApiKey(request: Request) {
    ...
    if (configuredApiKeys.length === 0 || isPublicRoute(request.path)) return;
    const providedApiKey = request.headers['x-api-key'] || request.headers['X-API-Key'];
    if (_.isString(providedApiKey) && configuredApiKeys.includes(providedApiKey.trim())) return;
    throw new Exception(EX.SYSTEM_API_KEY_INVALID).setHTTPStatusCode(HTTP_STATUS_CODES.UNAUTHORIZED);
}
